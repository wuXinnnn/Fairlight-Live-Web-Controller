...
12. **Fairlight 的 Ember+ provider 每约 500 ms 只 accept 一条连接,且收到 FIN 后永不关闭它那一端。** 2026-09-19 只读实测:同时发起 6 个握手,完成时间 4 / 513 / 1012 / 1513 ms,另两个的 SYN 被丢弃、靠重传 21 s 后才连上;我们 `socket.end()` 之后它那侧永久停在 CLOSE_WAIT,进程句柄数每会话加一,只有重启 Fairlight Live 才清掉。`resetAndDestroy()`(RST)后系统层面不留 CLOSE_WAIT,句柄仍约以一半速度增长。所以对它的短连接必须少开(探测周期 60 s、事件触发合并、最小间隔 5 s)、用 RST 关;多个后端连同一台时每个后端都在分它的 accept 配额,`Could not connect … after a timeout of 5 seconds` 多半是排队排输了而不是地址错(库把 ECONNREFUSED 也打成这句,已在 `connectFailureReason` 区分)。另外探测连接的 GetDirectory 在第一包 children 到达时就 resolve,总线目录分包发送时会少列条带,探测后要等 200 ms 尾包。
