...
- 通道条底部读数:电平读数(`MTR`)与电平值读数(`LVL`)不待在电平表列与推子列里,而是各占一整行、横跨整条通道条——读数的宽度因此与它上面那个控件无关。`.channel-strip__controls` 是一张「电平表井 / 推子轨道 / 刻度」三列、「控件行 + 两个读数行」的网格,`.meter` 与 `.fader` 用 `display: contents` 摊平,把各自的部件交给这一张网格(元素本身没有盒子,所以「无电平时变暗」的 `opacity` 挂在电平表井与读数上,而不是挂在 `.meter` 上)。两行读数共用一套「标签 / 值 / 单位」三列规则,**只有值那一列是弹性的**:数字向左伸展,到单位的距离是一个固定的 `margin`,位数增减都不会改变它,两行的单位也因此对齐;两行共用一条边框,合起来是一块仪表面板。两块读数在样式上分开:电平值可以点开输入,所以它的值格常驻一圈字段边框与下沉底色、悬停提亮、输入态换成通道色、锁定或断线时退色——与安全区页码同一套写法,理由也一样(触摸屏没有 hover,常驻边框是「可点」的唯一提示);电平读数只是读数,不加盒子。两者用同一份内边距(只读那份带一圈透明边框),数字因此仍收在同一条竖线上
