...
- **窗口的样子**:与混音页同一套「工业控制台」语言——Barlow Condensed 做标题、IBM Plex Mono 做全部小标签
  ...
  动作条),可缩放,默认 640×600、最小 560×440——只有中间那一行滚动,文档本身从不滚动。
- **两个启动复选框各管一件事**:`Start with Windows` 只注册或注销当前用户的自启项(`tauri-plugin-autostart`,
  不带任何参数);登录后显示窗口还是只剩托盘图标,由 `Start hidden in the tray` 决定,与从开始菜单启动时
  完全一致。命令行 `--hidden` 只对当次启动生效,自启项不用它。
