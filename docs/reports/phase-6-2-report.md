...
- **dnd-kit 结构**(最终形态,中间演变见第 11 节):`ViewDndContext.tsx` 承载 `DndContext`(包住 `.view-editor__grid` 两列),`ChannelOrderList.tsx` 的根 `SortableContext` items 为非空块(`channel:<rowKey>` / `group:<id>`),`SortableGroupBlock.tsx` 对非空组同时 `useSortable(group:<id>)` 与 `useDroppable(groupzone:<id>)`(两个 id 分开是因为 `useSortable` 已用项 id 注册 droppable),内嵌成员 `SortableContext`;`EmptyGroupBlock` 只 `useDroppable`,不进根 items;拖动通道时分组边界处另有 `RootSlot`(`slot:<position>`)droppable;`AvailableChannelList.tsx` 每个 label 用 `useDraggable(available:<channelId>)`,已勾选或保存中 `disabled`,把手按钮同时 `disabled`。两个 `SortableContext` 都用不位移任何行的 `previewSortingStrategy`:拖动期间列表按预览视图渲染,DOM 顺序本身就是预览。每个 draggable/droppable 的 `data` 带 `{ kind, label, groupId?, empty?, position?, current? }`(`dnd-ids.ts`),碰撞与播报按 data 判断而不解析 id。`onDragOver` / `onDragMove` 只更新预览视图(`syncPreview`),不改草稿;组块的 `is-drop-target` 由 `useDndContext()` 的 `over` 推导。`onDragEnd` 解析落点:`dragSourceFor` 把 active id 映射回拖动开始时的草稿(过期 key 返回 null → 无操作),`resolveDropTarget`(`drop-resolver.ts`)按统一的中线规则把 `over` 解析为 `DropTarget`(指针在行或组块中线之上落到其前、之下落到其后;`groupzone` 按组块中线取组首/组尾;`slot` 直接给出根级位置;键盘同列表下移落后、上移落前,跨列表落前),位置一律按去掉被拖项后的视图计数;有预览时经 `settlePreview` 结算后写入草稿,并以 `source === startView` 保证草稿在拖动期间未变。available 落下前再用 `assignedChannelIds` 拦一次。
...
- **碰撞检测与传感器**(`dnd-collision.ts`、`dnd-config.ts`):先按 `isEligibleTarget` 过滤(通道/available 源 → 通道行或组容器;组源 → 根单行或其它组,永远不能进入别的组;被拖项自身的 droppable 始终可落,供 sortable 策略在占位上方时回落到自身)。有指针坐标时用 `pointerWithin`:根级落槽优先于它覆盖的组头,行优先于所在组容器,只命中容器时按中线取组首/组尾,指针在列表外松手 → `over` 为空 → 不改草稿(超过移除阈值则移除);键盘路径排除非空组容器与被拖行已占的落槽后用 `closestCenter`。传感器:`PointerSensor(distance: 4)`、`TouchSensor(delay: 250, tolerance: 8)`、`KeyboardSensor(coordinateGetter: viewKeyboardCoordinates)` 三者同时启用(共存的实际行为见第 8 节);`viewKeyboardCoordinates` 把被拖项中心对齐方向上最近的可落目标中心(见第 8 节偏离说明),来自 AVAILABLE 的项在尚未进入列表时先跳到首/末目标;包装后的键盘坐标函数还记录方向键方向供落点提示使用。窗口级 `pointermove` / `touchmove` 只在指针或触屏来源的拖动中挂载,键盘拖动不受闲置鼠标影响。`saving` 为真时所有 `useSortable` / `useDraggable` `disabled` 且把手 `disabled`。`accessibility.announcements` 与 `screenReaderInstructions` 改为带通道名的英文文案。
- **FLIP hook**(`use-flip-list.ts`):`useFlipList(containerRef, dependency)` 在 `useLayoutEffect` 中读取容器内全部 `[data-flip-key]` 的 rect 与上次比对,位移 ≥ `FLIP_MIN_SHIFT_PX` 的元素先写反向 `translate`(`transition: none`,强制 reflow),`requestAnimationFrame` 后清 transform 并写 `FLIP_TRANSITION`,`transitionend` 或 `FLIP_CLEANUP_FALLBACK_MS` 兜底移除内联样式;成员行减去所属组块的位移,避免整组移动时双重平移;新出现的元素与首次测量不补间;`window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches` 为真不补间(jsdom 无 `matchMedia` 视为不减少)。hook 挂在 `SettingsPage` 的列表 ref 上,依赖为 `activeDraft`,所有草稿变更走同一套补间。与 dnd-kit 的配合:所有 `useSortable` 传 `animateLayoutChanges: () => false` 与 `transition: null` 关闭其布局动画;`onDragEnd` 提交草稿前调用 `flip.capture()` 把拖动态的 rect 记为基线,落下时行本身几乎零位移,被拖项的克隆由 `DragOverlay` 的 `dropAnimation` 飞向目标行。`skipNext()` 作为备选保留。注意依赖只有 `activeDraft`:拖动期间按预览视图重渲染不会触发 FLIP,占位行移动时其它行是瞬间换位。
...
4. 再拖一行到组内成员之间(把手压在成员行上半部分松开落其前、下半部分落其后),再拖到组头上松开,再拖到组块底边上松开。预期:前者落在指定位置,组头落为组首,组底落为组尾;同一列表内重排也是同样的上下半区规则,占位行随指针实时移动;悬停时组块有琥珀描边(`is-drop-target`)。
...
7c. 让一个分组后面紧跟一两个独立行,再放一个相邻的第二个分组。分别从上方和从下方把一个通道行拖到:第一组末位(指针压在该组最后一个成员的下半部分)、该组之后的第一个独立行(指针压在紧随其后那一行的上半部分)、第二组组首(指针压在组头上)。预期:三个落点从两个方向都能直接到达,不需要先越过再折返;占位行一旦出现,指针停在占位行上不再变化;松手后的落点与占位一致。
...
```text
eeb71d6 fix(web): keep keyboard drops on the arrow direction while a mouse rests
a231f93 docs: describe the midline drop rule and the fourth feedback round
dfcbd20 fix(web): resolve drops from the pointer midline in every list
acfeeda docs: describe the root slots at group boundaries
1d4441e feat(web): drop channels before, between and after groups with root slots
9577311 docs: record the second Bugbot review round in the Phase 6.2 report
e2b7c85 fix(web): keep the previewed slot on group drops and play the drop animation
a26548e docs: describe the drag overlay, placeholders, drag-out removal and edge scrolling
bbe5581 feat(web): drag overlay with drop placeholders, drag-out removal and edge scrolling
48cf4d6 feat(web): add drag preview, removal and edge auto-scroll building blocks
178dfe1 docs: record the Bugbot review fixes in the Phase 6.2 report
149481a fix(web): undo a rejected history traversal in the direction it came from
18d5555 docs: add the Phase 6.2 execution report
...
以上提交经 PR #15 squash 合并到 `main`(`52dbd11`)。

远端 CI 与 Cursor Bugbot 在最终提交 `eeb71d6` 上均为 `success`;首个全绿 run 为 `24ad698` 的 run #204(<https://github.com/wuXinnnn/Fairlight-Live-Web-Controller/actions/runs/34144556523>)。
...
用户本机验证第四轮提出两项:两组相邻时把通道条拖到第一组末位的判定区极窄;分组后面跟着独立通道条时,拖成「组后首个独立行」只能从下往上、拖成「组末位」只能从上往下。用 Playwright 逐 2px 扫描指针并记录预览 DOM 与 `over`(桌面 1400×900,行高约 85px、组头 50px),复现并定位到三个成因:

- **组头一律追加到组尾**:自上而下经组头进组时占位被放到组尾,被拖行原位消失使整组上移 85px,指针随即落在组首成员上,视觉与目标来回跳。
- **两套落点语义冲突**:跨列表按指针在行的上/下半区决定前后;同列表改用 dnd-kit 的 `verticalListSortingStrategy` +「占据被悬停行的槽位」(`arrayMove` 语义)。从下方在 Anagram-Wet 下半区进组(放到其后 = 组末位)后,同列表规则对同一位置给出「放到其前」;自上而下在 MIC 上半区脱组(放到其前 = 组后首行)后,同列表规则给出「放到其后」。于是这两个目标各只能从一个方向到达,另一方向需先越过边界再折返,用户感受为「判定区极窄」。
- **同一行内跨中线不更新**:预览只在 `onDragOver`(`over` 变化)时更新,指针在同一行内从上半区移到下半区不触发,松手时却按下半区结算,落点与占位不一致。

处理(`fix(web): resolve drops from the pointer midline in every list`):

- **单一中线规则**:`resolveDropTarget` 对同列表与跨列表统一——指针在行(或组块)中线之上落到其前、之下落到其后;`groupzone` 按组块中线取组首/组尾(组头 → 组首,组底 → 组尾,空组两者相同);组拖动对根级单行与其它组块同样按中线取前后。位置一律按去掉被拖项后的视图计数。`dropHintFor(pointer, overRect, keyboardDown, sameList)` 产生提示:指针/触屏读指针坐标(此前用克隆中心,受抓取点偏移影响);键盘保持既有语义(同列表下移落到目标行之后、上移落到之前,跨列表落到目标行之前),方向由包装后的键盘坐标函数记录,集成测试预期不变。
- **全程以预览视图渲染**:两个 `SortableContext` 改用不位移任何行的 `previewSortingStrategy`,`useSortable` 关闭自带 `transition`,同列表重排也改 DOM 顺序并由 FLIP 补间;预览在 `onDragMove` 与 `onDragOver` 上都同步(`syncPreview`,`previewFor` 无变化即不更新);新增 `DroppableRemeasure` 在每次按预览视图重渲染后全量重测 droppable(根列表重排会推移下方各组成员,dnd-kit 只重测变动列表自身的项,否则最多 300ms 内矩形过期)。该模型自洽:「放到 X 之后」仅当指针在 X 中线之下,移动后指针要么落在占位行上、要么仍在 X 中线同侧,不会震荡;两个方向的迟滞都是半行且与方向无关。`settlePreview` 保留为安全网(松手时通常解析为原位)。
- 验证:单测按新语义更新 `resolveDropTarget`(同列表上下半区、组容器首/尾、组拖动前后)与 `settlePreview`,新增 `dropHintFor`;集成测试新增指针传感器用例(`tests/pointer-drag.ts` 驱动 dnd-kit 指针传感器,`pointerUp` 后需等过 dnd-kit 50ms 的点击拦截再点 SAVE):`[g1(MAIN, FX)] BASS SUB` 从下往上先到 BASS 上半区(组后首行)再到 FX 下半区(组末位)并落下;`SUB [g1(MAIN, FX)] BASS` 从上往下经组头(组首)、MAIN/FX 下半区(逐位下移到组末位)、BASS 上半区(组后首行)并落下;既有键盘用例只有一处断言随「同列表移动即改 DOM」调整。Chromium 冒烟 `smoke5`(两种布局 × 两个方向逐 2px 扫描):组末位、组后首行、组首、组尾在两个方向的入口带均 ≥ 40px(约半行,进入后占位行本身 85px 内悬停稳定),两组之间与列表首位的入口带为落槽的 `ROOT_SLOT_HEIGHT_PX`;自上而下松手落为组末位与组后首行均正确;占位行上连续采样 DOM 稳定。`smoke2` / `smoke4` / `smoke-mobile` 无回归,`smoke3` 用例 A 的预期改为「落点 = 松手时显示的占位」(组头现在是组首,占位在途中已移到组首)。全量门通过:web 246 个测试,覆盖率语句 96.11 / 分支 90.64 / 函数 98.28 / 行 96.01。布局桩(`tests/stub-layout.ts`)给列表矩形上下各留 `STUB_LIST_PADDING = 80px`:jsdom 接受任意 `scrollTop`,指针停在首末行时若落在自动滚动边缘区,每帧都会滚动并全量重测,开启覆盖率时一次激活即超时。

Cursor Bugbot 对 `a231f93` 给出一条 finding(Medium):**键盘拖动的落点会跟随鼠标位置**。成立:窗口 `pointermove` / `touchmove` 监听对所有拖动都挂,键盘拾起时 `pointerRef` 为 null,但鼠标一动就被填上,`dropHintFor` 便按鼠标中线而非方向键判断,下移可能落到目标行之前。修复:`DragState` 记录 `pointer`(拾起事件带指针坐标才为 true),只在指针/触屏拖动中挂这两个监听;键盘拖动中 `pointerRef` 恒为 null,提示只取方向键,自动滚动也不再响应闲置的鼠标。集成测试在键盘拾起后先向 window 派发一次落在目标行上半区的 `pointermove`,再 ArrowDown,断言仍落到目标行之后(旧代码上该断言失败)。
