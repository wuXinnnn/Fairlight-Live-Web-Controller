...
**第二轮**:机器人对上面第二条的修复又报了一条,也属实,由 `<clone-fix>` 修复。我把组 id 从 `startView` 取出后,仍然拿这个 id 去**预览视图**里取组对象;而 `groupAccent` 在组没有覆盖色时是按**当前成员**算众数的,所以被拖的成员一旦预览出组,组的自动色就变了,克隆的颜色照样在指针下面跳。我上一轮写的回归用例用的是带 `color: 'teal'` 覆盖色的组——`groupAccent` 见到覆盖色就直接返回,**根本走不到成员那条路**,所以那条用例对这个缺陷是瞎的。现在组对象整个从 `startView` 取(成员也是拾起时的那一份),与注释里承诺的「保持拾起时的颜色」一致;新增用例用**没有覆盖色**的组(2 aux + 1 main → navy,拖走一个 aux 后平局归 main → red),把修复移除后它确实变红,守得住。
