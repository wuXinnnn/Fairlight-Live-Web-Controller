...
第三轮的三条修订同样**没有新增任何数值**:末尾落槽的悬停高亮原样复用 `.view-group.is-drop-target` 的三行声明(`1px solid var(--amber)` / `outline-offset: -1px` / `#171610`),即将删除的占位复用 `var(--red)` 与 `.drag-overlay.is-removing` 的底色 `#2a1716`,判定距离仍是既有的 `REMOVE_DRAG_THRESHOLD_PX`。
...
3. 把一行拖到列表**下方的空白区域**(最后一行以下、列表框以内)松手。预期:落在最后。以前那里是落空的。指针进入空白区时,预期**整片区域**出现琥珀描边 + 微暗底(和悬停在一个组上的高亮一样);预期**看不到**任何横贯空白区的琥珀虚线——组与组之间那些细落槽的虚线仍然在,那是第三轮改的(见第 13 节)。
4. 从左侧 AVAILABLE 拖一个未勾选通道到同一片空白区。预期:追加为最后一行,左侧勾选框同步勾上。
5. **空 view 任意位置皆可落**(第三轮改的):新建一个 view,一个通道都不勾。从 AVAILABLE 拖一个通道进来,分别试**列表最上方(提示文字原来所在的那一带)**、中间、下方。预期:三处都能落进去。以前只有越过那条虚线以下才行。拖动一开始,`THIS VIEW HAS NO CHANNELS` 提示应当消失;放弃这次拖动(Esc)后它回来。
...
- **第三轮的第 2 条改动是纯 CSS,本会话无法自动化验证**。jsdom 不求值样式,选择器写没写对、末尾落槽的整片高亮好不好看,都只能在真机上看(第 5 节鼠标路径第 3 步)。第 1、3 两条的行为由集成测试守住,但它们的**观感**同样只能真机确认。
...
**改法**:把虚线限定在非 `fill` 的落槽上(`.root-slot:not(.root-slot--fill) .root-slot__band::before`),`fill` 落槽改为命中时**整片高亮**,原样复用组容器已有的投放语汇(`.view-group.is-drop-target` 的三行声明)。判断依据是两者性质不同:边界落槽没有自己的高度,一条线就是它的全部;末尾落槽是一片**区域**而不是一条边界,画一条线横过去只会读成把空白区切成两半。组与组之间那些细落槽的虚线**保留**——去掉它们就完全不可见了。
