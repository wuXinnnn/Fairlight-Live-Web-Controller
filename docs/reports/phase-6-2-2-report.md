...
五项任务全部落地:FLIP 补间可续接并改用无 transform 几何、View 数据模型改为有序块(配置版本 2,版本 1 读取时迁移)、列表末尾常驻占满式落槽、组色 AUTO 取成员类型众数、文档同步。分 7 个提交推送到 `claude/blissful-mccarthy-6uzsqt`,其后又有 CI、评审与真机验收带来的修订(见第 9、10、13 节),分支上共 17 个提交。
...
| 5. 全量质量门:串行 lint → typecheck → test → build 全绿,远端 CI 全绿,lockfile 无 diff | **部分通过** | 本地:`eslint .` 与 `prettier --check .` 全绿;`@flwc/shared` 与 `@flwc/web` 的 typecheck / test / build 全绿(截至第三轮修订:web 327 例、shared 44 例)。`apps/server` 与 `packages/test-utils` **本地无法安装依赖,未运行**。`git diff pnpm-lock.yaml` 为空。远端 CI 结果见第 9 节。 |

**覆盖率**(本地可跑的两个包):

- `apps/web`:语句 96.75% / 分支 91.98% / 函数 99.20% / 行 96.70%(门槛 80%,数字为第三轮修订后的最新一次)
...
第三轮的三条修订同样**没有新增任何数值**:即将删除的占位复用 `var(--red)` 与 `.drag-overlay.is-removing` 的底色 `#2a1716`,判定距离仍是既有的 `REMOVE_DRAG_THRESHOLD_PX`;当时给末尾落槽加的悬停高亮复用了 `.view-group.is-drop-target` 的三行声明,已在第四轮连同边界落槽的虚线一起删除(见第 14 节)。第四轮是**净删除**,同样无新增数值。
...
3. 把一行拖到列表**下方的空白区域**(最后一行以下、列表框以内)松手。预期:落在最后。以前那里是落空的。**拖动全程不应出现任何琥珀虚线或区域高亮**(第四轮把根级落槽的视觉全删了,见第 14 节):判断落点一律看**占位行滑到哪**。唯一还会亮的是组块——指针悬停在某个组上时该组仍有琥珀实线描边。被拖行自身的占位描边也仍是琥珀色(拖出容器时变红),那是另一套,不要混。
4. 从左侧 AVAILABLE 拖一个未勾选通道到同一片空白区。预期:追加为最后一行,左侧勾选框同步勾上。
5. **空 view 任意位置皆可落**(第三轮改的):新建一个 view,一个通道都不勾。从 AVAILABLE 拖一个通道进来,分别试**列表最上方(提示文字原来所在的那一带)**、中间、下方。预期:三处都能落进去。以前只有越过那条(现已删除的)虚线以下才行。拖动一开始,`THIS VIEW HAS NO CHANNELS` 提示应当消失;放弃这次拖动(Esc)后它回来。
6. **拖出删除的占位**(第三轮改的):按住某一行把手,把指针拖到 CHANNEL ORDER 列表**框外** 64px 以上(往下或往旁边都行)。预期:跟手的克隆变红并显示 `DROP TO REMOVE`,**同时列表里那一行的占位也从琥珀虚线框变成红色虚线框 + 深红底**;把指针拖回列表内,两者都恢复琥珀。松手即删除该行。组头同样试一次,预期整个组块的占位变红,松手连成员一起删除。
7. `NEW GROUP` 建一个组(它出现在列表末尾),**不要**放任何通道进去。用组头的箭头把它上下移动;再按住组头把手整组拖动。预期:都能动——空组现在和别的块一样有位置。
8. 把一个通道拖到那个空组**之后**(空组下方的空白区)。预期:成为独立的无组行,不会被吸进组里。再拖到空组**之前**,预期:成为它上面的一行。
9. 按住某个组的组头把手,拖到列表下方的空白区松手。预期:整组移到末尾。
10. **组色**:建一个组,放进 1 个 main + 2 个 aux。预期:组头与跟随组色的成员行都是 aux 的颜色(navy)。把其中一个 aux 拖出组。预期:变成 main 的颜色(red)——1 比 1 平局时取成员里最先出现的类型。
11. `SAVE VIEW` 回到混音页切到该 view。预期:分组段标题颜色与配置页组头一致。
12. 若系统开启「减弱动态效果」,重复第 2 步。预期:无补间,直接到位。
...
**第二轮**:机器人对上面第二条的修复又报了一条,也属实,由 `3470841` 修复。我把组 id 从 `startView` 取出后,仍然拿这个 id 去**预览视图**里取组对象;而 `groupAccent` 在组没有覆盖色时是按**当前成员**算众数的,所以被拖的成员一旦预览出组,组的自动色就变了,克隆的颜色照样在指针下面跳。我上一轮写的回归用例用的是带 `color: 'teal'` 覆盖色的组——`groupAccent` 见到覆盖色就直接返回,**根本走不到成员那条路**,所以那条用例对这个缺陷是瞎的。现在组对象整个从 `startView` 取(成员也是拾起时的那一份),与注释里承诺的「保持拾起时的颜色」一致;新增用例用**没有覆盖色**的组(2 aux + 1 main → navy,拖走一个 aux 后平局归 main → red),把修复移除后它确实变红,守得住。

三条都是**本地跑不了 server 依赖、也跑不了浏览器冒烟**之外的另一类补网:静态评审。第一条说明「迁移的失败模式」值得单独断言,而不是只断言成功路径;第三条说明**回归用例本身要挑能走到出错分支的夹具**——用带覆盖色的组去测一个只在自动色下出现的缺陷,等于没测。

**一处有意的行为取舍**:旧实现里克隆的颜色是跟着预览的成员变化走的(旧 `leadKindOf` 也扫当前视图)。现在克隆在整段拖动里颜色固定为拾起时的样子。指针底下的克隆在拖动途中改颜色是干扰而不是信息,组头与列表本身仍然实时反映预览,所以信息没有丢。
...
- **纯样式的改动本会话一律无法自动化验证**。jsdom 不求值 CSS。第三轮的第 2 条如此,**第四轮整轮如此**(它只删样式,没有任何行为变化,因此一条新测试都没有)。第 5 节鼠标路径第 3 步是它的验收步骤。第三轮第 1、3 两条的行为由集成测试守住,但它们的**观感**同样只能真机确认。
...
| `3470841` | `fix(web): freeze the drag clone's colour at the moment of pickup`(同上,第二轮) |
| `1079bcb` | `docs: record the second review round in the Phase 6.2.2 report` |
| `548c91f` | `fix(web): make the whole of an empty view a drop target`(真机验收,见第 13 节) |
| `7eda4ee` | `fix(web): stop drawing a dashed line across the empty area below the list`(同上) |
| `3e3aed6` | `feat(web): show that a dragged-out item is about to be removed`(同上) |
| `a38cb68` | `docs: record the hardware acceptance fixes in the Phase 6.2.2 report` |
| `c7bb04e` | `fix(web): let the placeholder alone show where a drop will land`(真机验收第四轮,见第 14 节) |
...
用户在真机上验收后提出三条问题,全部属实,全部落在本批次改动的落槽/拖拽这一块,由 `548c91f`、`7eda4ee`、`3e3aed6` 三个提交修复。

### 13.1 拖进空 view 时,列表顶部是死区

**现象**:把通道拖进一个空 view,停在「首个通道条的位置」落不进去;必须先往下越过那条琥珀虚线,再拉回来,或者干脆落在虚线以下。

**根因**:空 view 的 `<ol>` 第一个子元素是 `<li class="panel-empty">THIS VIEW HAS NO CHANNELS</li>`,它**不是 droppable**,却占着约 3rem —— 正好是第一行的位置。常驻的 `fill` 落槽是 `flex: 1 1 auto`,只能从提示文字的下沿开始吃剩余空间。碰撞检测走 `pointerWithin`(严格几何命中,不吸附最近的容器),所以指针停在提示文字上时 `over === null`,`syncPreview` 直接返回,松手也什么都不发生。**这条提示本身是 6.2.1 引入的**(「列表容器始终渲染,空态文案改为列表内的一行」),6.2.2 把 `fill` 落槽改成常驻之后,它就成了列表里唯一一块没有落点的地方。

**改法**:拖动期间不渲染这行提示(`ChannelOrderList.tsx`,条件加 `&& !dragging`)。提示一让位,`<ol>` 里就只剩 `fill` 落槽,它自然吃满整个列表,从顶边到底边是同一个落点;拖动结束且仍为空时提示回来。

**测试**:`settings-dnd.integration.test.tsx` 新增「把通道落在空 view 第一行本该在的位置」。**先确认它在修复前是红的**——那个坐标上落下得到 `[]`,jsdom 里能原样复现这个 bug;修复后通过。同一例还断言拖动一开始提示就不在文档里。

### 13.2 空白区上那条琥珀虚线

用户问这条线是不是本批次加的。**规则本身不是**:`.root-slot__band::before`(`2px dashed`,悬停变实心)是 Phase 6.2(`52dbd11`)为组与组之间那些**零高度**的 16px 落槽画的,那是它们唯一可见的东西。**但这个观感是本批次造成的**:`1488ec8` 把 `fill` 落槽从「只在空草稿时出现」改成「整段拖动常驻」,于是同一条规则在空白区顶边画出了一条横贯的分割线。

**改法**:把虚线限定在非 `fill` 的落槽上(`.root-slot:not(.root-slot--fill) .root-slot__band::before`),`fill` 落槽改为命中时**整片高亮**,原样复用组容器已有的投放语汇(`.view-group.is-drop-target` 的三行声明)。判断依据是两者性质不同:边界落槽没有自己的高度,一条线就是它的全部;末尾落槽是一片**区域**而不是一条边界,画一条线横过去只会读成把空白区切成两半。组与组之间那些细落槽的虚线**保留**——去掉它们就完全不可见了。**这个理由在第四轮被推翻**:落槽不可见不等于落点不可见,占位行一直在指示落点。见第 14 节。

**这一条本会话无法自动化验证**:jsdom 不求值 CSS。移交用户按第 5 节鼠标路径第 3 步看。

### 13.3 拖出容器待删除时,列表里的占位没有删除语汇

**现象**:把通道行拖出 view 容器,松手前跟手的克隆已经变红并显示 `DROP TO REMOVE`,但列表里留下的占位仍是普通的琥珀虚线框,看不出松手会删掉它。

**改法**(用户在两个方案里选了「给占位一个即将删除的样式」):`removing` 本来就在 `DragPreviewState` 里,只是没有走到列表。现在把它挂成列表容器上的一个 class(`.view-channel-list.is-removing`),行与组块的占位靠后代选择器改成红色虚线框 + 深红底(`var(--red)` 与 overlay 同款的 `#2a1716`),指针回到列表内即恢复琥珀。容器级一个 class 就够,不必把状态钻进每一行。

**一处有意的取舍**:占位仍停在离开列表前最后一个合法落点,**不回弹到原位**。回弹更「诚实」(松手确实是按起拖时的视图删除),但指针在容器边缘进出很频繁,每进出一次都会让列表抽动一下、触发一次 FLIP 补间,比停住更难用。

**顺带补上一个真实的覆盖缺口**:改之前全仓**没有任何测试**断言「拖出列表 → 删除」或 overlay 变红——`grep "is-removing\|DROP TO REMOVE"` 在测试里 0 命中,只有 `pointerOutside` / `removalFor` / `dropAnimationFor` 三个纯函数级用例,`ViewDndContext` 里把它们串起来的那两段从未被测过。本轮补了两例(行与组),覆盖「拖出 → 标记 → 拖回 → 恢复 → 再拖出 → 松手删除」的完整往返,并做了反向验证:把 `is-removing` 那处改动移除后,两例确实变红。

### 13.4 这一轮说明的问题

三条里有两条(13.1、13.2)是**同一个根因的两面**:`fill` 落槽从「偶尔出现」变成「常驻」是一次性质上的改变,而它周围那些为「偶尔出现」写的东西——空态提示的位置、边界落槽的虚线——没有跟着一起重新想过。改动本身是对的,漏的是「改完之后,这块区域现在长什么样」。这类问题在单元测试与集成测试里都照不出来:第一条要有人真的把指针停在列表最上面一格,第二条根本不是行为而是观感。真机验收在这个批次里是**必要的一环,不是形式**,与第 10 节那三条静态评审发现的缺陷合起来,本批次总共有 7 个问题是在本地质量门全绿之后才被发现的。

## 14. 真机验收后的修订(第四轮)

用户试过第三轮的改动后提出两点,都是对**拖拽视觉语汇**的继续收敛,由 `c7bb04e` 一个提交完成。两点合起来是同一件事:**根级落槽不再有任何视觉,落点只由预览的占位行指示**。

### 14.1 末尾落槽的整片高亮说不通,删掉

**现象**:view 底部有空白时,指针停在空白区里**占位行所在的那一格**不高亮,再往下一点才高亮。

**根因**:预览一旦把占位行放到列表末尾,那一行就占住了空白区的顶部,`fill` 落槽从它下面才开始——指针命中的是占位行(它本身也是一个 droppable)而不是落槽,`isOver` 为假。**两处落点的结果完全一样**(都是追加到末尾),高亮却忽明忽暗。这是第三轮那个设计自带的毛病,不是实现出错:只要落槽的"整片"会被它自己产生的占位行盖住一截,高亮就不可能一致。

**改法**:删掉 `.root-slot--fill .root-slot__band.is-over`。占位行滑到末尾本身就说明了落点,高亮是把同一件事说第二遍,而且说得不一致。

### 14.2 组间的琥珀虚线一并删掉

第三轮保留边界落槽虚线的理由是「去掉它们就完全不可见了」(第 13.2 节)。用户要求推翻这个理由试一试,**理由确实站不住**:落槽不可见不等于落点不可见——命中边界落槽时,占位行就出现在那个根级位置上,指示一直都在。删掉 `.root-slot__band::before` 与它的 `is-over` 变体。

至此根级落槽是纯粹不可见的命中带。`RootSlot` 里 `useDroppable` 的 `isOver` 随之没有任何用途,一起删掉。DOM 结构与内联几何一个字没动(`li.root-slot[.root-slot--fill]` + `div.root-slot__band[data-root-slot]`,band 仍是 `firstElementChild`),所以 `stub-layout.ts` 的落槽分支与所有按 `data-root-slot` / `.root-slot` 选元素的用例继续有效。

### 14.3 保留的与偏离的

**保留**:组块的琥珀实线描边(`.view-group.is-drop-target`)。它回答的是另一个问题——"进组还是落在组上方",这两种结果的占位行只差一行高度,光看占位读不出来;而落槽标的是"根级的哪个位置",占位行落在哪就是哪,没有歧义。它与落槽是两套独立机制:描边由 `SortableGroupBlock` 的 `useIsDropTarget` 从 `useDndContext().over` 派生,而落槽的 `DndItemData` 里根本没有 `groupId`,永远点不亮组块。第三轮的 fill 高亮是**复制**了这条规则的三行声明,删的是副本,原件没动。被拖项自身的占位描边(`.channel-order-row.is-dragging`,拖出容器时转红)也是另一套,不受影响。

**规格偏离**:`docs/prompts/phase-6-2-2.md` 第 146 行写着「虚线指示保持在槽的顶边,`is-over` 高亮不变」。本轮按真机反馈推翻了这一条。提示词是历史规格文件,按惯例不回改,偏离记在这里。

### 14.4 这一轮的验证状况(要如实看)

**本轮没有新增任何测试,一条都没有**。改动只删样式与一个再没人用的 `isOver`,唯一可断言的"少了一个 class"没有价值;全仓 `is-over` 在测试里本来就 0 引用,所以既有的 327 例一条都没受影响(跑过,全绿,覆盖率未降)。**代价是这一轮的正确性完全没有自动化背书**——jsdom 不求值 CSS,删对没删对、删完好不好看,只能在真机上看。第 5 节鼠标路径第 3 步是它的验收步骤。

还有一点要请用户重点判断:**把通道拖到两个组之间、或首组之前,现在那里完全没有视觉提示**,只有占位行会出现在该位置。这是本轮把可发现性压得最低的地方。如果真机上觉得难用,把边界落槽的虚线单独加回来是一步就能退的——`.root-slot:not(.root-slot--fill) .root-slot__band::before` 那条规则,加回来即可,末尾落槽的高亮不必跟着回来(它本来就说不通)。
