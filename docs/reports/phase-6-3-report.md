...
| † `STRIP_MIN_HEIGHT_PX` | 452 | 通道条的最小高度,低于它降级为页内滚动;它决定混音台不滚动所需的最矮窗口(见 14.9) | 同上 |
...
| `54892eb` | `fix(web): close the gap between a channel name and its ON key`(用户实测,14.9) |
| `be896e3` `83fec4b` | `adjust(web): …` 两步把最矮窗口从 672px 降到 596px(用户实测平板,14.9) |
...
### 14.9 通道名与 ON 键之间的空档,以及平板上被截掉的底部

**空档**。`.channel-strip__header` 被钉成固定的 `4.15rem`(66.4px),而里面的东西(上边距 13.6 + 信号条 2 + 8 + 一行名称 16.5)一共只有 40.1px,剩下的四分之一是死空间;加上 ON 键自己的 `margin-top: 0.8rem`,名称底边到 ON 顶边一共 **39.1px**。固定高度原本是为了让相邻通道条的 ON 键对齐,但名称是 `white-space: nowrap` + 省略号、**永远只有一行**,按内容撑开本来就一样高。改为按内容撑开、内边距 `0.8rem / 0.6rem`,ON 的上边距 0.8 → 0.55rem:页头 66.4 → **49.9px**,名称到 ON **39.1 → 19.4px**。

**平板底部被截**。先把机制查清楚:**不是内容溢出通道条**。把通道条高度逐档强压下去实测,一直压到 **200px** 内容才开始画出条外——`STRIP_MIN_HEIGHT_PX` 原来的 528 离那个下限差得远,在 528 时条内还余 13.8px。真正发生的是**页在滚动**:窗口不够高时通道条停在下限,段(下限 + 标题栏 26 + 间距 6)加上页的上下内边距超过可用高度,页于是自己滚——而滚动条是隐藏的(12.1 的几何取舍),底部那行电平读数就这么无声地落到折叠线以下。

所以这个数只能往**下**调,不能往上:它逐像素决定「混音台不滚动所需的最矮窗口」。实测这条对应关系是 1:1——

| `STRIP_MIN_HEIGHT_PX` | 开始滚动的窗口高度 |
| --- | --- |
| 528(原) | 672px |
| 511 | 652px |
| 452(现) | **596px** |

先降到 511:页头省下的 16.5px 正是当初标定 528 时被算进去的死空间,所以 511 的通道条与原来 528 的**一样宽松**(推子行程不变)。用户选定再降到能盖住 600px 窗口,于是取 452。此时下限态的推子行程 242px,条内仍余 13.8px。16 档扫描里 **1440×600 与 800×600 从滚动 36 / 52px 变成完全不滚**;只有 520 高的两档仍然滚(452 + 32 + 28.8 + 页头页脚 已经超过 520)。交互 13 项全过。
