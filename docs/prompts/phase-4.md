...
按 `docs/development-plan.md` 的 Phase 4 与 `docs/architecture.md` 的前端结构实现混音页 MVP。本阶段**不做 views 配置**(Phase 5)、**不做深度视觉打磨与触屏优化**(Phase 6),但必须建立全站固定深色主题(不支持深浅色切换),样式需简洁、贴近真实调音台直觉、少解释性文本。交互和状态组件需有简短、合理的动效避免生硬,拖动保持即时跟手,非必要动效兼容 reduced-motion。
...
- 所有前端页面固定使用深色主题,通过全局设计 token 统一背景、文字、边框与状态色;不跟随系统浅色偏好,不实现主题状态或切换入口。
- 推子非拖动收敛、ON、按钮、连接提示、reset 确认及通道条带增删应使用克制的短动效;不得让动效延迟直接操作或电平实时反馈,并通过 `prefers-reduced-motion` 缩减非必要动画。
...
- 必须覆盖 `docs/conventions.md` 前端边界场景清单中本阶段适用项 + Phase 4 验收标准点名场景:
  ...
  - 全局固定深色基线且无主题切换入口;通道 presence/reset 确认等计时动效状态可正确清理。
...
3. **实现摘要** — 状态层(store 拆分、patch 合成、重连恢复)、推子(pending 态与收敛策略、发送频率)、ON 开关、电平表(峰值保持与颜色分段参数、避免整页重渲的具体做法)、响度区、断线降级、固定深色主题与组件动效(reduced-motion 处理)各自的关键设计点,以及与 `docs/architecture.md` 不一致之处。
